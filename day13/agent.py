"""Day 13: Agent with Task State Machine in Working Memory."""
...
SYSTEM_INSTRUCTIONS = (
    "Ты ассистент с трёхслойной памятью и конечным автоматом задачи. "
    "USER PROFILE задаёт КАК отвечать. "
    "CURRENT TASK (если есть) — source of truth по цели, stage, current_step "
    "и expected_action. Не меняй stage самостоятельно и не проси заново "
    "объяснить задачу, если Goal уже известен из CURRENT TASK. "
    "Если Status=paused — напомни, что задача на паузе, пока пользователь "
    "не возобновит её. Если пользователь пишет «Продолжай» при active задаче — "
    "продолжай с Current step / Expected action."
)

DEMO_GOAL = "Подготовить технический план REST API"
...
class Agent:
    """
    Day 11 memory + Day 12 profile + Day 13 TaskState (Working Memory).

    TaskState is persisted under TASK_STATE_KEY and injected into every
    LLM request as CURRENT TASK. Stage transitions only via TaskState API.
    """
    ...
    def build_system_content(self) -> str:
        ...
        parts.append(
            self.memory.format_working_block(exclude_keys={TASK_STATE_KEY})
        )
        return "\n".join(parts)
